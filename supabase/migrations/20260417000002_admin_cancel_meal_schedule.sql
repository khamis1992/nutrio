...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM public.profiles WHERE id = v_admin_id AND role IN ('admin', 'super_admin')
  ) THEN
  ...
  IF v_active_job_count > 0 THEN
    RAISE EXCEPTION 'Cannot cancel: a driver is already preparing or delivering this order.';
  END IF;
  ...
  IF v_schedule.addons_total IS NOT NULL AND v_schedule.addons_total > 0 THEN
    ...
    INSERT INTO public.wallet_transactions (
      ...
      'order',
      p_schedule_id,
      'Admin cancellation refund - addons for ' || COALESCE(v_schedule.meal_name, 'meal'),
    ...
    IF v_schedule.restaurant_id IS NOT NULL THEN
      ...
      IF v_restaurant.owner_id IS NOT NULL THEN
        INSERT INTO public.notifications (
          ...
          'An admin has cancelled the order for ' || COALESCE(v_schedule.meal_name, 'a meal') || '.',
          'unread',
          jsonb_build_object(
  ...
  BEGIN
    INSERT INTO public.notifications (
      ...
      v_schedule.user_id,
      'order_update',
      'Meal Cancelled by Admin',
      'Your scheduled meal for ' || COALESCE(v_schedule.meal_name, 'a meal') || ' has been cancelled by admin. Your meal credit has been refunded.',
      'unread',
      jsonb_build_object(
        'schedule_id', p_schedule_id,
        'meal_name', COALESCE(v_schedule.meal_name, ''),
