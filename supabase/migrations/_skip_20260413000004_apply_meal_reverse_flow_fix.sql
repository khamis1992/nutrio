-- Migration: Apply meal reverse flow fixes
-- Date: 2026-04-13
-- Description: Creates/updates atomic delete function and XP deduction function for meal reverse flow
-- This migration uses CREATE OR REPLACE so it can be applied independently of migration history

-- Function to deduct XP when meal is deleted (reverse of award_xp_for_meal_log)
DROP FUNCTION IF EXISTS deduct_xp_for_meal_deletion(UUID, INTEGER);
...
DROP FUNCTION IF EXISTS delete_meal_entry_atomic(UUID, UUID, TEXT, INTEGER, INTEGER, INTEGER, INTEGER, TIMESTAMPTZ, UUID, DATE, INTEGER);
