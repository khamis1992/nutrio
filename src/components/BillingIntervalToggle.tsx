...
export function BillingIntervalToggle({
  ...
  return (
    <div className="flex items-center justify-between rounded-[18px] bg-slate-50 p-4 ring-1 ring-slate-200/80">
      <div className="flex items-center gap-3">
        <div className={`rounded-full p-2 ${isAnnual ? "bg-emerald-50 text-emerald-600 ring-1 ring-emerald-100" : "bg-white text-slate-500 ring-1 ring-slate-200/80"}`}>
          <Calendar className="h-5 w-5" />
        </div>
        <div>
          <div className="flex items-center gap-2">
            <Label htmlFor="billing-toggle" className="cursor-pointer font-bold text-slate-950">
              {t("annual_billing")}
            </Label>
  ...
  return (
    <div className={`space-y-1 ${className}`}>
      <div className="flex items-baseline gap-2">
        <span className="text-lg font-semibold text-emerald-600">{annualPrice.toLocaleString()} QAR</span>
