    ...
    if (tokens.expires_at * 1000 < Date.now()) {
      try {
        ...
        return await fetchGoogleFitWorkouts(
          { accessToken: refreshResult.access_token, expiresAt: refreshResult.expires_at },
          startDate,
          endDate
        );
      } catch (err) {
        console.error("Failed to refresh Google Fit token:", err);
        return [];
      }
    }
