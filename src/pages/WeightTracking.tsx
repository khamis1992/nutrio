    ...
    <div className="min-h-screen bg-muted/50 pb-24">
      ...
      <div className="px-4 py-5 space-y-4">
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm">
          <p className="text-sm font-semibold text-muted-foreground mb-1">{t('current')}</p>

          <div className="flex items-baseline gap-3 mb-4">
            <span className="text-5xl font-black text-foreground">{currentWeight?.toFixed(1) ?? "--"}</span>
            <span className="text-lg font-semibold text-muted-foreground">{t('kg')}</span>
            {change != null && (
              <span className={cn("flex items-center gap-1.5 text-sm font-semibold", change <= 0 ? "text-primary" : "text-destructive")}>
                <span className={cn("w-5 h-5 rounded-full flex items-center justify-center", change <= 0 ? "bg-primary" : "bg-destructive")}>
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          ...
          {displayed.length === 0 ? (
            <p className="text-sm text-muted-foreground px-5 pb-5">{t('no_entries_yet')}</p>
          ) : (
                ...
                return (
                  <div key={entry.id} className="flex items-center px-5 py-3.5 relative">
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-foreground">{entry.weight_kg.toFixed(1)} {t('kg')}</p>
                      <p className="text-xs text-muted-foreground mt-0.5">{formatEntryDate(entry.log_date, t)}</p>
                    </div>

                    {diff != null && (
                      <span className={cn("flex items-center gap-1.5 text-sm font-semibold mr-3", diff <= 0 ? "text-primary" : "text-destructive")}>
                        <span className={cn("w-6 h-6 rounded-full flex items-center justify-center shrink-0", diff <= 0 ? "bg-primary" : "bg-destructive")}>
      ...
      {sheetOpen && (
        <div className="fixed inset-0 z-[200] flex flex-col justify-end">
          ...
          <div className="relative bg-white rounded-t-3xl flex flex-col overflow-hidden" style={{ maxHeight: "92vh", marginBottom: 84 }}>
            ...
            {/* Title */}
            <h2 className="flex-none text-center text-lg font-bold text-foreground py-3">{t('update_weight')}</h2>
            ...
            <div className="flex-1 overflow-y-auto px-4 pb-2">
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          selected ? "bg-primary" : "hover:bg-muted/50"
                        )}
                      >
                        <span className={cn("text-xs font-medium", selected ? "text-white/80" : "text-muted-foreground")}>
                          {format(d, "EEE")}
                        </span>
                        <span className={cn("text-base font-bold mt-0.5", selected ? "text-white" : "text-foreground")}>
                          {format(d, "d")}
                        </span>
                        {selected && <span className="w-1.5 h-1.5 rounded-full bg-primary/30 mt-1" />}
                      </button>
                    );
                  })}
                </div>
              ...
              {/* Weight input */}
              <div className="bg-muted/50 rounded-2xl flex items-center justify-center gap-3 py-6">
                <input
                  type="number"
                  min="20"
                  max="300"
                  ...
                  className="text-6xl font-black text-foreground tracking-tight bg-transparent border-none outline-none w-40 text-center"
                />
                <span className="text-2xl font-semibold text-muted-foreground">{t('kg')}</span>
              </div>
            </div>

            {/* Cancel / Save — pinned to bottom with safe space */}
            <div className="flex-none flex gap-3 px-4 pt-3 pb-5 bg-white border-t border-muted">
              <button
                onClick={() => setSheetOpen(false)}
                className="flex-1 rounded-full border-2 border-primary text-primary font-bold text-base hover:bg-primary/10 transition-all"
                style={{ height: 52 }}
              >
                {t('cancel')}
              </button>
              <button
                ...
                className="flex-1 rounded-full bg-primary hover:bg-primary/90 disabled:opacity-50 text-white font-bold text-base transition-all active:scale-[0.98]"
                style={{ height: 52 }}
              >
