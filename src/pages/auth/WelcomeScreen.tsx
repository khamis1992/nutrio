...
export const WelcomeScreen = ({ onSwitchView }: WelcomeScreenProps) => {
  ...
  const GoogleIcon = (<svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M23.04 12.261c0-.815-.073-1.596-.209-2.348H12v4.44h6.24c-.27 1.45-1.09 2.676-2.32 3.497v2.9h3.76c2.2-2.027 3.46-5.015 3.46-8.49Z" fill="#4285F4"/><path d="M12 24c3.24 0 5.96-1.074 7.947-2.917l-3.76-2.9c-1.044.7-2.385 1.114-4.187 1.114-3.215 0-5.94-2.17-6.914-5.09H1.2v3.02C3.176 21.41 7.28 24 12 24Z" fill="#34A853"/><path d="M5.086 14.207A7.2 7.2 0 0 1 4.8 12c0-.765.132-1.505.37-2.207V6.773H1.2A11.96 11.96 0 0 0 0 12c0 1.908.46 3.708 1.2 5.227l3.886-3.02Z" fill="#FBBC05"/><path d="M12 4.727c1.764 0 3.35.607 4.598 1.795l3.448-3.448C17.953 1.18 15.24 0 12 0 7.28 0 3.176 2.59 1.2 6.773l3.972 3.02C6.06 6.897 8.785 4.727 12 4.727Z" fill="#EA4335"/></svg>);
  const AppleIcon = (<svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M16.365 1.43c0 1.085-.43 2.136-1.13 2.9-.857.93-2.258 1.648-3.432 1.552-.12-1.072.472-2.213 1.176-2.95.83-.87 2.287-1.54 3.386-1.502zM20.75 18.12c-.65 1.507-1.43 2.993-2.58 4.314-1.008 1.158-2.27 2.46-3.89 2.46-1.63 0-2.062-.79-3.83-.79-1.794 0-2.26.78-3.867.81-1.632.03-2.864-1.245-3.877-2.405C.93 20.1-.65 15.46 1.82 12.1c1.028-1.45 2.625-2.37 4.46-2.4 1.676-.03 3.26.85 4.28.85 1 0 2.84-1.05 4.79-.9.82.03 3.12.33 4.59 2.5-3.89 2.23-3.27 7.02.8 5.97z"/></svg>);
  const FacebookIcon = (<svg width="22" height="22" viewBox="0 0 24 24" fill="#1877F2"><path d="M22.675 0H1.325C.593 0 0 .593 0 1.326v21.348C0 23.406.593 24 1.325 24H12.82V14.706H9.692v-3.61h3.13V8.413c0-3.1 1.894-4.788 4.66-4.788 1.325 0 2.463.099 2.793.143v3.24l-1.918.001c-1.504 0-1.795.715-1.795 1.764v2.314h3.588l-.467 3.61h-3.12V24h6.116C23.407 24 24 23.406 24 22.674V1.326C24 .593 23.407 0 22.675 0Z"/></svg>);
  ...
  return (
    <div className="fixed inset-0 flex flex-col overflow-hidden bg-white dark:bg-gray-950" style={{ maxWidth: 430, margin: "0 auto" }}>
      ...
      <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} transition={{ type: "spring", damping: 24, stiffness: 220, delay: 0.05 }} className="flex flex-col bg-white dark:bg-gray-900" style={{ borderTopLeftRadius: 36, borderTopRightRadius: 36, marginTop: -12, zIndex: 10, paddingTop: 28, paddingLeft: 20, paddingRight: 20, paddingBottom: "max(28px, env(safe-area-inset-bottom, 28px))", boxShadow: "0 -4px 16px rgba(0,0,0,0.04)", backgroundImage: "radial-gradient(1200px 100px at 50% -60px, rgba(0,0,0,0.03) 0%, rgba(0,0,0,0) 60%)" }}>
        ...
        <div className="flex items-center gap-3 mt-6 mb-4"><div className="h-px bg-slate-200 flex-1" /><span className="text-[12px] text-slate-400">Or continue with</span><div className="h-px bg-slate-200 flex-1" /></div>
        <div className="flex items-center justify-center gap-4">
          {[{ key: "google", icon: GoogleIcon }, { key: "apple", icon: AppleIcon }, { key: "facebook", icon: FacebookIcon }].map((s) => (
            <button key={s.key} type="button" className="w-12 h-12 rounded-full bg-white ring-1 ring-slate-200 shadow-[0_1px_3px_rgba(15,23,42,0.04)] hover:shadow-[0_4px_12px_rgba(15,23,42,0.08)] transition-shadow flex items-center justify-center" aria-label={s.key}>{s.icon}</button>
          ))}
        </div>

        <p className="mt-7 text-[12px] text-center text-slate-400">By continuing, you agree to our <span className="mx-1" /><Link to="/privacy" className="text-emerald-600 hover:underline">{t("privacy_policy")}</Link><span className="mx-1">|</span><Link to="/terms" className="text-emerald-600 hover:underline">{t("terms")}</Link></p>
      </motion.div>
    </div>
  );
};
