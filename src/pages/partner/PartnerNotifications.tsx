...
const PartnerNotifications = () => {
  ...
  const markAsRead = async (id: string) => {
    ...
    await supabase.from("notifications").update({ status: "read", read_at: readAt }).eq("id", id);
    setNotifications((prev) => prev.map((n) => (n.id === id ? { ...n, status: "read", read_at: readAt } : n)));
  };
  const markAllAsRead = async () => {
    if (!user) return;
    const readAt = new Date().toISOString();
    await supabase.from("notifications").update({ status: "read", read_at: readAt }).eq("user_id", user.id);
    setNotifications((prev) => prev.map((n) => ({ ...n, status: "read", read_at: readAt })));
    ...
    <PartnerLayout title="Notifications" subtitle="Order alerts and updates">
      <div className="space-y-4">
        ...
        {notifications.length === 0 ? <Card><CardContent className="py-12 text-center"><Bell className="h-12 w-12 mx-auto mb-4 text-muted-foreground/50" /><p className="text-muted-foreground">No notifications yet</p></CardContent></Card> : notifications.map((notification) => (
          <Card key={notification.id} className={`transition-colors ${isUnread(notification) ? "bg-primary/5 border-primary/20" : ""}`}>
            <CardContent className="p-4"><div className="flex gap-4"><div className="w-10 h-10 rounded-full bg-muted flex items-center justify-center shrink-0">{getIcon(notification.type)}</div><div className="flex-1 min-w-0"><div className="flex items-start justify-between gap-2"><div><p className="font-semibold">{notification.title}</p><p className="text-sm text-muted-foreground mt-1">{notification.message}</p><p className="text-xs text-muted-foreground mt-2">{notification.created_at ? new Date(notification.created_at).toLocaleString() : ""}</p></div><div className="flex gap-1 shrink-0">{isUnread(notification) && <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => markAsRead(notification.id)}><Check className="h-4 w-4" /></Button>}<Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive" onClick={() => deleteNotification(notification.id)}><Trash2 className="h-4 w-4" /></Button></div></div></div></div></CardContent>
          </Card>
        ))}
      </div>
    </PartnerLayout>
