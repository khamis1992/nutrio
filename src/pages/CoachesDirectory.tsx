  ...
  const fetchCoaches = useCallback(async () => {
    try {
      ...
      const [{ data: profiles }, { data: assignments }] = await Promise.all([
        supabase
          .from("profiles")
          .select("user_id, full_name, avatar_url, goal_type")
          .in("user_id", coachIds),
        supabase
      ...
      const coachRatings = new Map<string, { sum: number; count: number }>();
      ...
      const sorted: CoachProfile[] = (profiles || [])
        .map((p) => ({
          ...
          rating: 4.5,
          verified: true,
          goalTypes: p.goal_type ? [p.goal_type] : [],
        }))
        .sort((a, b) => b.clientCount - a.clientCount);

      setCoaches(sorted);
      if (myActiveCoach.size > 0) setMyCoach([...myActiveCoach][0]);
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] pb-20">
      <div className="max-w-[480px] mx-auto px-4 py-5">
        ...
        {hasCoach && (
          <div className="mb-5 rounded-2xl bg-violet-50 border border-violet-200 p-4 flex items-center gap-3">
            <CheckCircle2 className="w-5 h-5 text-violet-600 shrink-0" />
            <p className="text-sm font-semibold text-violet-800">
              You're connected with a coach. They can view your progress.
            </p>
          </div>
        )}
