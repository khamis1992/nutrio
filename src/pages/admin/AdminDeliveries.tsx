...
export default function AdminDeliveries() {
  ...
  const completionRate =
    completionTotal > 0
      ? Math.round((stats.delivered / completionTotal) * 100)
      : 0;

  return (
    <AdminLayout
      title="Delivery Management"
      subtitle="Manage all delivery operations"
    >
      <div className="space-y-6">
        <AdminWorkbenchHeader
          eyebrow="Live dispatch"
          title="Delivery control desk"
          icon={Route}
          accent="#38BDF8"
          description="Assign pending orders, monitor active handoffs, and keep online drivers moving through one dispatch-focused workspace."
          meta={[
            { label: "Pending", value: pendingJobs.length },
            { label: "Active", value: activeJobs.length },
            { label: "Drivers", value: onlineDrivers.length },
          ]}
          actions={
            <>
              <Button
                variant="outline"
                className="h-11 rounded-[14px] border-[#E5EAF1] bg-white px-5 font-black text-[#020617] shadow-[0_10px_24px_rgba(2,6,23,0.045)] hover:bg-[#F6F8FB]"
                onClick={fetchData}
              >
              ...
              <Button
                ...
                className="h-11 rounded-[14px] border-[#38BDF8]/30 bg-[#38BDF8]/10 px-5 font-black text-[#020617] shadow-[0_12px_24px_rgba(2,6,23,0.06)] hover:bg-[#38BDF8]/15"
              >
                <Truck className="mr-2 h-4 w-4 text-[#38BDF8]" />
        ...
        <section className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-5">
          ...
          <StatsCard
            ...
            color={C.protein}
            description="Assigned or moving"
          />
          <StatsCard
            title="In Transit"
        ...
        <section className="rounded-[24px] bg-white p-4 shadow-[0_14px_34px_rgba(2,6,23,0.06)] ring-1 ring-[#E5EAF1]">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              <div className="grid h-11 w-11 place-items-center rounded-[16px] bg-[#22C7A1]/10 text-[#22C7A1] ring-1 ring-[#22C7A1]/15">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Pill
                label={`${onlineDrivers.length} drivers online`}
                color={C.progress}
              />
            </div>
          </div>
        ...
        <Tabs defaultValue="pending" className="space-y-4">
          <TabsList className="grid h-auto w-full grid-cols-3 rounded-[22px] bg-white p-1.5 shadow-[0_10px_26px_rgba(2,6,23,0.045)] ring-1 ring-[#E5EAF1]">
            <TabsTrigger
              value="pending"
              className="rounded-[18px] py-3 text-sm font-black data-[state=active]:bg-[#38BDF8]/10 data-[state=active]:text-[#020617] data-[state=active]:ring-1 data-[state=active]:ring-[#38BDF8]/25"
            >
...
function StatsCard({
  ...
  return (
    <AdminPanel className="rounded-[22px]">
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          <div
            className="grid h-11 w-11 place-items-center rounded-[16px] bg-[#F6F8FB] ring-1 ring-[#E5EAF1]"
            style={{ color }}
          >
            {icon}
          </div>
          <p className="text-3xl font-black tracking-tight text-[#020617]">
            {value}
          </p>
        </div>
        <p className="mt-4 text-[11px] font-black uppercase tracking-[0.12em] text-[#94A3B8]">
          {title}
        </p>
...
function PendingDeliveriesTable({
  ...
  if (jobs.length === 0) {
    return (
      <EmptyState
        ...
        description="All deliveries have been assigned to drivers."
        color={C.progress}
      />
    );
  }
  ...
  return (
    <AdminPanel className="overflow-hidden rounded-[24px]">
      <div className="p-0">
        <SectionTitle
          icon={<Clock className="h-5 w-5" />}
            ...
            {jobs.map((job) => (
              <DeliveryRow key={job.id} job={job} accent={C.water}>
                <DriverDialog
                  title="Assign Driver"
                  onlineDrivers={onlineDrivers}
                  onSelect={(driverId) => onAssign(job.id, driverId)}
                />
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-11 w-11 rounded-full text-[#FB6B7A] hover:bg-[#FB6B7A]/10 hover:text-[#FB6B7A]"
                  onClick={() => onCancel(job.id)}
                  aria-label="Cancel delivery assignment"
                >
                  <X className="h-4 w-4" />
                </Button>
              </DeliveryRow>
  ...
  if (jobs.length === 0) {
    return (
      <EmptyState
        ...
        description="Drivers are currently not on active delivery jobs."
        color={C.protein}
      />
    );
  }
    ...
    <AdminPanel className="overflow-hidden rounded-[24px]">
      <div className="p-0">
        <SectionTitle
          ...
          description="Assigned and in-transit jobs currently moving."
          color={C.protein}
        />
        <ScrollArea className="h-[560px]">
    ...
    <AdminPanel className="overflow-hidden rounded-[24px]">
      <div className="p-0">
        <SectionTitle
          ...
          description="Available drivers with live status and recent performance."
          color={C.progress}
        />
        <ScrollArea className="h-[560px]">
...
function DeliveryRow({
  ...
  const waitingMinutes = Math.max(
    0,
    differenceInMinutes(new Date(), new Date(job.created_at)),
  );
  ...
  return (
    ...
    >
      <div className="flex min-w-0 items-center gap-3">
        <div
          className="grid h-12 w-12 shrink-0 place-items-center rounded-[16px] bg-[#F6F8FB] shadow-[0_10px_24px_rgba(2,6,23,0.045)] ring-1 ring-[#E5EAF1]"
          style={{ color: accent }}
        >
        ...
        <div className="min-w-0">
          <p className="truncate text-sm font-black text-[#020617]">
            {job.schedule.meal.name}
          </p>
          <p className="mt-1 text-xs font-semibold text-[#94A3B8]">
            #{job.id.slice(-6)} - {job.schedule.meal_type}
          </p>
        </div>
      </div>
      ...
      <InfoBlock
        title="Customer"
        value={job.schedule.user?.raw_user_meta_data?.name || "Customer"}
        meta={job.schedule.user?.email || "No email"}
      />
      <InfoBlock
        title="Restaurant"
        value={job.schedule.meal.restaurant?.name || "Restaurant"}
        meta={job.schedule.meal.restaurant?.address || "Address not set"}
      />

      <div className="flex flex-wrap items-center gap-2 lg:justify-end">
        <Badge className="rounded-full border-[#38BDF8]/20 bg-[#38BDF8]/10 px-3 py-1 font-black text-[#38BDF8]">
...
function ActiveDeliveryCard({
  ...
  const isPickedUp = ["picked_up", "in_transit", "on_the_way"].includes(
    job.status,
  );
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0 space-y-4">
          <div className="flex flex-wrap items-center gap-2">
            <Badge
              className="rounded-full border-transparent bg-white px-3 py-1 font-black ring-1 ring-[#E5EAF1]"
              style={{ color: accent }}
            >
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 truncate text-xs font-semibold text-[#94A3B8]">
                {job.schedule.meal.restaurant?.name || "Restaurant"}
              </p>
            </div>
          </div>
  ...
  return (
    <div className="rounded-[22px] border border-[#E5EAF1] bg-[#F6F8FB] p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-1 rounded-full bg-[#22C7A1]/10 px-2.5 py-1 text-xs font-black text-[#22C7A1]">
      ...
      <div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-2">
        <MiniMetric
          label="Vehicle"
          value={driver.vehicle_type || "Vehicle"}
          icon={
        ...
        <MiniMetric
          label="Earned"
          value={`${driver.total_earnings || 0} QAR`}
    ...
    <Dialog>
      <DialogTrigger asChild>
        {trigger || (
          <Button
            variant="outline"
            className="h-11 rounded-[14px] border-[#38BDF8]/30 bg-[#38BDF8]/10 px-4 font-black text-[#020617] hover:bg-[#38BDF8]/15"
          >
            Assign Driver
          </Button>
        )}
      ...
      <AdminDialogContent size="md">
        <DialogHeader className="border-b border-[#E5EAF1] bg-[#F6F8FB] p-5 text-left">
          <DialogTitle className="text-xl font-black tracking-tight text-[#020617]">
            {title}
          </DialogTitle>
        </DialogHeader>
        <div className="max-h-[360px] space-y-2 overflow-y-auto p-5">
          {onlineDrivers.length === 0 ? (
            <p className="rounded-[18px] bg-[#F6F8FB] py-8 text-center text-sm font-semibold text-[#94A3B8]">
...
function SectionTitle({
  ...
  return (
    <div className="flex items-center gap-3 border-b border-[#E5EAF1] bg-[#F6F8FB] p-4">
      <div
        className="grid h-11 w-11 place-items-center rounded-[16px] bg-white ring-1 ring-[#E5EAF1]"
        style={{ color }}
      >
        {icon}
      </div>
      <div>
        <h2 className="text-xl font-black tracking-tight text-[#020617]">
          {title}
        </h2>
...
function InfoBlock({
  ...
  return (
    <div className="min-w-0 rounded-[16px] bg-[#F6F8FB] p-3">
      <p className="text-[11px] font-black uppercase tracking-[0.08em] text-[#94A3B8]">
        {title}
      </p>
  ...
  return (
    <AdminPanel className="rounded-[24px]">
      <div className="px-6 py-14 text-center">
        <div
          className="mx-auto mb-4 grid h-16 w-16 place-items-center rounded-[24px] bg-[#F6F8FB] ring-1 ring-[#E5EAF1]"
          style={{ color }}
        >
          {icon}
        </div>
...
function driverName(
  driver: Pick<OnlineDriver, "user"> | NonNullable<DeliveryJob["driver"]>,
) {
